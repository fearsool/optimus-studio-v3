...
import fs from 'fs';
import { Critic } from '../../../services/Critic'; // Import at top level
...
export async function POST() {
    ...
    try {
        ...
        const searchPath = path.join(rootDir, 'automations');
...
`;
    ...
    try {
        ...
        const filesToScan = [
            ...
            path.join(process.cwd(), 'src/app/ui/dashboard/SelfHealer.tsx'),
            path.join(process.cwd(), 'src/app/services/Doctor.ts'),
            path.join(process.cwd(), 'src/app/api/doctor/scan/route.ts'),
            path.join(process.cwd(), 'src/app/patient.ts') // Added test file
        ];

        for (const filePath of filesToScan) {
            if (fs.existsSync(filePath)) {
                const content = fs.readFileSync(filePath, 'utf-8');
                const metrics = critic.analyze(path.basename(filePath), content);

                // Thresholds: Complexity > 10 is warning, > 20 is error
                if (metrics.cyclomaticComplexity > 15) {
                    const relativeName = path.relative(process.cwd(), filePath); // FIX: Use relative path
                    diagnoses.push({
