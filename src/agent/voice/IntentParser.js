...
exports.IntentParserTool = exports.IntentParser = void 0;
...
class IntentParser {
    ...
    async detectLanguage(text) {
        ...
        try {
            const langCode = (0, franc_1.franc)(cleanText, { minLength: 3 });
            if (langCode === 'tur')
                return 'turkish';
            if (langCode === 'eng')
                return 'english';
    ...
    async ruleBasedAnalysis(text, language) {
            ...
            for (const keyword of keywords) {
                ...
                // Benzerlik (Levenshtein distance)
                    ...
                    for (const word of words) {
                        if (word.length >= 3) {
                            const distance = natural_1.default.LevenshteinDistance(keyword, word, { search: true });
    ...
    async keywordBasedAnalysis(text, language) {
        const words = text.split(' ');
        const scores = {};
    ...
    async machineLearningAnalysis(text, language) {
    ...
    async aiBasedAnalysis(text, language) {
        try {
            const prompt = `
      ...
      `;
            const response = await this.modelRouter.query('intent_analysis', prompt, {
                model: 'gpt-4-turbo'
            });
            let parsedResponse;
            try {
    ...
    selectBestIntent(analyses) {
        ...
        const emergencyAnalysis = analyses.find(a => a.intent === 'emergency' &&
            a.confidence > 0.7 &&
            a.method === 'ai_gpt');
    ...
    // PARAMETRE ÇIKARIMI
    async extractParameters(text, intent, language) {
        const params = {};
    ...
    async extractParametersWithAI(text, intent) {
        try {
            const prompt = `
      ...
      `;
            const response = await this.modelRouter.query('parameter_extraction', prompt, {
                model: 'gpt-4-turbo'
            });
            try {
                const jsonMatch = response.content.match(/\{[\s\S]*\}/);
        ...
        for (const [level, indicators] of Object.entries(urgencyIndicators)) {
            for (const indicator of indicators) {
                if (lowerText.includes(indicator)) {
                    ...
                    const newLevel = levelOrder[level];
    ...
    // EYLEM BELİRLEME
    determineAction(intent, parameters) {
    ...
    // ONAY GEREKLİLİĞİ
    requiresApproval(intent, parameters) {
        ...
        if (intent === 'system_control') {
            ...
            if (criticalActions.some(action => { var _a; return (_a = parameters.action) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(action); })) {
                return true;
            }
        }
        return false;
    ...
    async generateVoiceResponse(intent, parameters, language) {
        const responses = {
            turkish: {
    ...
    async generateConfirmationMessage(originalText, intentResult) {
    ...
    // YARDIMCI METODLAR
    normalizeNumbersAndSymbols(text) {
        return text
            .replace(/\$(\d+)/g, '$1 dolar')
            .replace(/€(\d+)/g, '$1 euro')
            .replace(/₺(\d+)/g, '$1 lira')
