...
export type TaskType = 'coding' | 'planning' | 'turkish' | 'long_context' | 'video_script' | 'creative';
...
export class ModelOrchestrator {
    ...
    private modelRegistry: Map<TaskType, ModelConfig>;
    ...
    async routeRequest(task: TaskType, prompt: string, context?: any): Promise<string> {
        const config = this.modelRegistry.get(task);
        if (!config) throw new Error(`No model configured for task: ${task}`);
...
Bağlam: ${context?.audience || 'genel'} [/INST]`;
...
Style: ${context?.style || 'Energetic'} [/INST]`;
