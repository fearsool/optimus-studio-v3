...
// AI AGENTS (Groq)
// ============================================
async function executeAI(config: any, context: any, apiKeys: any): Promise<NodeExecutionResult> {
    const groqKey = apiKeys.GROQ_API_KEY || process.env.GROQ_API_KEY;
    if (!groqKey) return { success: false, output: null, error: 'GROQ_API_KEY missing' };
    ...
    try {
        ...
        if (!response.ok) return { success: false, output: null, error: `Groq Error: ${response.status}` };
...
// MEDIA AGENTS (Fal.ai)
// ============================================
async function executeMedia(config: any, context: any, apiKeys: any): Promise<NodeExecutionResult> {
    const falKey = apiKeys.FAL_API_KEY || process.env.FAL_API_KEY;
    if (!falKey) return { success: true, output: { skipped: 'FAL_API_KEY missing' } };
    ...
    try {
        ...
        if (!response.ok) return { success: false, output: null, error: `Fal Error: ${response.status}` };
        const data = await response.json();
...
// ============================================
async function executeTelegram(config: any, context: any, apiKeys: any): Promise<NodeExecutionResult> {
    ...
    if (!token || !chatId) return { success: true, output: { skipped: 'Telegram not configured' } };
...
// LOGIC AGENTS
// ============================================
async function executeLogic(config: any, context: any): Promise<NodeExecutionResult> {
...
// ============================================
const interpolate = (text: string, context: any) => {
    ...
    return text.replace(/\{\{(.*?)\}\}/g, (_, key) => {
        ...
        if (val === undefined && context.variables) val = context.variables[parts[0]];
        return val !== undefined ? String(val) : '';
    });
};

async function executeHttpRequest(config: any, context: any, apiKeys: any): Promise<NodeExecutionResult> {
...
export const nodeExecutors: Record<string, NodeExecutorFn> = {
    ...
    'logic_gate': executeLogic as any, // Simple wrapper
