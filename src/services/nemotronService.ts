...
export interface NemotronOutput {
    success: boolean;
    task_type: NemotronTaskType;
...
const STATIC_FALLBACKS: Record<NemotronTaskType, (input: Record<string, any>) => Record<string, any>> = {
...
class NemotronService {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (this.isAvailable) {
                    ...
                    const configuredExists = models.some((m: any) => m.id === this.modelName);
     ...
     */
    async executeTask(taskInput: NemotronInput): Promise<NemotronOutput> {
     ...
     */
    private buildPrompt(taskInput: NemotronInput): string {
        const taskPrompt = TASK_PROMPTS[taskInput.task_type] || '';
        const feedbackContext = feedbackService.getGlobalContext();
...
Respond ONLY with valid JSON. No markdown formatting, no explanations.`;
     ...
     */
    private parseOutput(response: string, taskType: NemotronTaskType): Record<string, any> {
        try {
     ...
     */
    private executeFallback(taskInput: NemotronInput, startTime: number): NemotronOutput {
    ...
    async evaluateAutomation(niche: string, purpose: string): Promise<NemotronOutput> {
    ...
    /** Kalite denetimi */
    async auditQuality(product: any): Promise<NemotronOutput> {
        return this.executeTask({
    ...
    /** Kişiselleştirme */
    async personalize(content: string, profile: string, sector: string): Promise<NemotronOutput> {
        return this.executeTask({
    ...
    /** Satış desteği */
    async assistSale(userMessage: string, context: any): Promise<NemotronOutput> {
        return this.executeTask({
    ...
    /** Fabrika kararı */
    async factoryDecision(situation: string, data: any): Promise<NemotronOutput> {
        return this.executeTask({
     ...
     * Get status
     */
    getStatus(): { available: boolean; initialized: boolean; lastCheck: number } {
        return {
...
export default nemotronService;
