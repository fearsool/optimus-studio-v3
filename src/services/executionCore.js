...
exports.ExecutionEngine = exports.ExecutionGraph = void 0;
...
class ExecutionGraph {
    constructor(blueprint) {
        this.nodes = new Map();
        this.edges = new Map(); // Source -> Edges
        this.incomingEdges = new Map(); // Target -> Edges
    ...
    addEdge(source, target, type = 'main') {
        var _a, _b;
        const edge = { source, target, type };
        if (!this.edges.has(source))
            this.edges.set(source, []);
        (_a = this.edges.get(source)) === null || _a === void 0 ? void 0 : _a.push(edge);
        if (!this.incomingEdges.has(target))
            this.incomingEdges.set(target, []);
        (_b = this.incomingEdges.get(target)) === null || _b === void 0 ? void 0 : _b.push(edge);
    }
...
class ExecutionEngine {
     ...
     */
    async transitionState(executionId, newState, reason) {
        try {
            const currentState = await supabaseService_1.default.getExecutionState(executionId);
            if (!currentState) {
                console.error(`[Engine] Execution ${executionId} not found`);
                return false;
            }
            const fromState = currentState;
            if (!(0, types_1.isValidTransition)(fromState, newState)) {
                console.error(`[Engine] Invalid transition: ${fromState} → ${newState}`);
                return false;
            }
            await supabaseService_1.default.updateExecutionState(executionId, newState, reason);
     ...
     */
    async startExecution(workflowId, blueprint, input, apiKeys = {}) {
        // 1. Init
        const executionId = await supabaseService_1.default.createExecution(workflowId, input);
        if (!executionId)
            return null;
        console.log(`[Engine] Starting Execution ${executionId} (V3.1 State Machine)`);
        // 2. Transition to RUNNING state
        await this.transitionState(executionId, types_1.ExecutionState.RUNNING, 'Execution started');
        ...
        if (graph.startNodeId) {
            ...
            await this.transitionState(executionId, types_1.ExecutionState.SUCCESS, 'No nodes to execute');
        }
        return executionId;
    }
    /**
     ...
     */
    async processEvent(executionId, blueprint = null) {
        var _a;
        // 1. Load State & Graph
        const state = await supabaseService_1.default.loadExecutionState(executionId);
        if (!state)
            throw new Error('Execution not found');
        // Fetch blueprint if missing
        if (!blueprint) {
            const bp = await supabaseService_1.default.getBlueprintById(state.workflowId);
            if (!bp)
        ...
        for (const nodeId of completedNodes) {
            ...
            for (const edge of edges) {
                ...
                if (edge.type === 'main')
                    allowed = true;
                else if (stepState.output && String(stepState.output._branch) === edge.type)
                    allowed = true;
                if (allowed)
        ...
        for (const candidateId of candidates) {
            ...
            const parentStats = incoming.map(e => {
                ...
                return ({
                    id: e.source,
                    state: ((_a = state.steps[e.source]) === null || _a === void 0 ? void 0 : _a.state) || 'PENDING',
                    output: (_b = state.steps[e.source]) === null || _b === void 0 ? void 0 : _b.output
                });
            });
            // Barrier Check
        ...
        for (const nodeId of allNodes) {
            const stepState = (_a = stepsData[nodeId]) === null || _a === void 0 ? void 0 : _a.state;
            if (stepState === 'RUNNING' || stepState === 'PENDING')
                runningCount++;
            else if (stepState === 'SUCCESS' || stepState === 'SKIPPED')
                successCount++;
        ...
        if (runningCount === 0 && pendingCount === 0) {
            if (failedCount === 0 && successCount > 0) {
                ...
                await this.transitionState(executionId, types_1.ExecutionState.SUCCESS, `All ${successCount} steps completed`);
            }
            else if (failedCount > 0 && successCount > 0) {
                // Some failed, some succeeded
                await this.transitionState(executionId, types_1.ExecutionState.PARTIAL_SUCCESS, `${successCount} success, ${failedCount} failed`);
            }
            else if (failedCount > 0 && successCount === 0) {
                // All failed
                await this.transitionState(executionId, types_1.ExecutionState.FAILED, `All ${failedCount} steps failed`);
            }
        }
    }
    /**
     ...
     */
    async dispatchNode(executionId, graph, nodeId, input, apiKeys = {}) {
        const node = graph.getNode(nodeId);
        if (!node)
            return;
        // 1. Calculate Input Hash
        const inputHash = (0, hashUtil_1.generateHash)(input);
        // 2. Check Idempotency (Has this run before with this input?)
        const existing = await supabaseService_1.default.checkStepIdempotency(executionId, nodeId, inputHash);
        if (existing) {
        ...
        const lockAcquired = await supabaseService_1.default.tryLockStep(executionId, nodeId, inputHash, 1);
        if (!lockAcquired) {
            console.log(`[Engine] Lock Contention (Skip): ${nodeId}`);
            return;
        }
        const { agentQueue } = await Promise.resolve().then(() => __importStar(require('./agentQueueService')));
     ...
     */
    async replayFromStep(executionId, stepId, blueprint) {
        var _a;
        try {
            console.log(`[Engine] Replaying from step ${stepId} in execution ${executionId}`);
            // 1. Reset the step (deletes existing record)
            const resetSuccess = await supabaseService_1.default.resetStepForReplay(executionId, stepId);
            if (!resetSuccess) {
            ...
            let mergedInput = ((_a = state.variables) === null || _a === void 0 ? void 0 : _a.$input) || {};
            if (incoming.length > 0) {
                // Get parent outputs
                const parentOutputs = {};
                for (const edge of incoming) {
                    const parentState = state.steps[edge.source];
                    if (parentState === null || parentState === void 0 ? void 0 : parentState.output) {
    ...
    async processExecution(executionId, blueprint = null) {
