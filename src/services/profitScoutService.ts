...
export interface ProfitOpportunity {
    ...
    source: 'github_trending' | 'producthunt' | 'rapidapi' | 'manual' | 'ai_generated';
    url?: string;
    tags: string[];
    score: number; // 1-100 profit potential score
    foundAt: string;
}
...
class ProfitScoutService {
     ...
     */
    async scanGitHubTrending(): Promise<ProfitOpportunity[]> {
        console.log('🔍 [ProfitScout] GitHub Trending taranıyor...');

        const newOpportunities: ProfitOpportunity[] = [];
            ...
            for (const query of queries) {
                try {
                    ...
                    for (const repo of data.items || []) {
                        ...
                        if (existingIds.has(`github-${repo.id}`)) continue;
     ...
     */
    private calculateProfitScore(name: string, description: string, stars: number): number {
        let score = 0;
    ...
    private guessCategory(name: string, description: string): string {
        ...
        if (text.includes('payment') || text.includes('stripe') || text.includes('invoice')) return 'money-maker';
        if (text.includes('ecommerce') || text.includes('shop') || text.includes('store')) return 'ecommerce';
        if (text.includes('social') || text.includes('instagram') || text.includes('tiktok')) return 'social-media';
        if (text.includes('ai') || text.includes('gpt') || text.includes('llm')) return 'assistant';
        if (text.includes('trading') || text.includes('crypto') || text.includes('bitcoin')) return 'crypto';
        if (text.includes('scrape') || text.includes('crawl')) return 'scraper';
     ...
     * Gelir tahmini
     */
    private estimateRevenue(score: number): string {
        if (score >= 80) return '₺50,000-200,000/ay';
        if (score >= 60) return '₺20,000-50,000/ay';
        if (score >= 40) return '₺5,000-20,000/ay';
     ...
     */
    opportunityToTemplate(opp: ProfitOpportunity): Partial<AutomationTemplate> {
        return {
            id: `scout-${opp.id}`,
            name: `🎯 ${opp.name}`,
     ...
     */
    async runFullScan(): Promise<{ added: number; total: number; skipped: number }> {
        ...
        const { getTemplates } = await import('./templateService');
        const factoryTemplates = await getTemplates();

        // Mevcut şablon isimlerini normalize et (küçük harf, tire yerine boşluk)
        const existingNames = new Set(
            factoryTemplates.map(t => t.name.toLowerCase().replace(/[-_]/g, ' ').replace(/\[ham\]/gi, '').trim())
        );
        ...
        let aiOpps: ProfitOpportunity[] = [];
        try {
            const { aiIdeaGeneratorService } = await import('./aiIdeaGenerator');
        ...
        const trulyNew = allFindings.filter(opp => {
            ...
            if (existingNames.has(normalizedName) || existingIds.has(scoutId)) {
                ...
                if (opp.source === 'ai_generated') return true;
        ...
        try {
            ...
            const existingOppIds = new Set(existing.map((o: any) => o.id));
     ...
     */
    getOpportunities(): ProfitOpportunity[] {
        try {
...
export const profitScoutService = new ProfitScoutService();
export default profitScoutService;
