...
export interface TestResult {
    passed: boolean;
    layer: 'TECHNICAL' | 'QUALITY' | 'SIMULATION';
    score: number; // 0-100
    warnings: string[];
    errors: string[];
    timestamp: string;
...
class AutomationTestService {
     ...
     */
    async runFullTestSuite(automation: any): Promise<AutomationTestSuite> {
    ...
    // --------------------------------------------------------
    private runTechnicalValidation(automation: any): TestResult {
        const errors: string[] = [];
        const warnings: string[] = [];

        // Check 1: Mandatory Fields
        if (!automation.name) errors.push('Missing Name');
        if (!automation.description) errors.push('Missing Description');
        if (!automation.workflowData) errors.push('Missing Workflow Data');

        // Check 2: Infinite Loop Risk
        const nodes = automation.workflowData?.nodes || [];
        if (nodes.length > 50) warnings.push('High complexity (50+ nodes)');

        // Check 3: Missing Connections
        // (Simplified logic for now)
        const isolatedNodes = nodes.filter((n: any) => !n.connected);
        if (isolatedNodes.length > 0) warnings.push(`${isolatedNodes.length} isolated nodes detected`);
    ...
    // --------------------------------------------------------
    private async runQualityAudit(automation: any): Promise<TestResult> {
        // Use Nemotron QUALITY_AUDIT task
        const audit = await nemotronService.executeTask({
    ...
    // --------------------------------------------------------
    private async runScenarioSimulation(automation: any): Promise<TestResult> {
    ...
    // HELPER
    // --------------------------------------------------------
    private compileSuite(
        id: string,
        automationId: string,
        tech: TestResult,
        qual: TestResult | null,
        sim: TestResult | null
    ): AutomationTestSuite {
        // Determine Verdict
        let verdict: 'PASS' | 'WARN' | 'FAIL' = 'PASS';

        if (!tech.passed) verdict = 'FAIL';
        else if (qual && !qual.passed) verdict = 'FAIL'; // Quality fail = reject
        else if (sim && !sim.passed) verdict = 'WARN';   // Sim fail might be soft
    ...
    private emptyResult(layer: any): TestResult {
