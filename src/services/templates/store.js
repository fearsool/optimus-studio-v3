...
exports.createBlueprintFromTemplate = exports.searchTemplates = exports.getTemplatesByCategory = exports.getTemplateById = exports.TEMPLATE_CATEGORIES = exports.AUTOMATION_TEMPLATES = exports.syncLocalTemplatesToDB = exports.getTemplates = exports.refineTemplate = exports.isTemplateSellable = exports.getBlockingReasons = void 0;
const types_1 = require("../../types");
const etsySeoListing_1 = require("./etsySeoListing");
const hairSalonBot_1 = require("./hairSalonBot");
const githubImportedTemplates_1 = require("./githubImportedTemplates");
...
const LEGACY_TEMPLATE_DEFAULTS = {
...
// REFINERY STATE MACHINE VALIDATION
const getBlockingReasons = (template) => {
    const reasons = [];
    // Helper to add system reasons
    const addReason = (msg) => reasons.push({ reason: msg, source: 'system', resolved: false });
    // 1. Basic Content
    if (!template.name)
        addReason('İsim eksik');
    if (!template.description)
        addReason('Açıklama eksik');
    // 2. Commercial Data (Strict check for REFINED status)
    if (template.refineLevel === 'refined') {
        if (!template.businessOutcome)
            addReason('İş Çıktısı (Outcome) Tanımı eksik');
        if (!template.monetizationType)
            addReason('Gelir Modeli belirsiz');
        // Official templates bypass signature check if not present (backwards compatibility)
        if (!template.signature && !template.isOfficial)
            addReason('İmza (Digital Signature) eksik - Lisanslanamaz');
    }
...
const isTemplateSellable = (template) => {
    ...
    if ((0, exports.getBlockingReasons)(template).length > 0)
        return false;
    return true;
};
exports.isTemplateSellable = isTemplateSellable;
// Fetches templates from DB mixed with local defaults
// --- REFINERY SERVICE ---
const refineTemplate = async (templateId, refinementData) => {
...
const getTemplates = async () => {
    ...
    let localStorageTemplates = [];
    try {
        const stored = localStorage.getItem('imported_templates');
        if (stored) {
            const parsed = JSON.parse(stored);
            localStorageTemplates = parsed.map((t) => ({
                ...t,
    ...
    const officialTemplates = [etsySeoListing_1.ETSY_SEO_LISTING_ENGINE, hairSalonBot_1.HAIR_SALON_APPOINTMENT_BOT];
    ...
    const legacyStatic = exports.AUTOMATION_TEMPLATES.map(t => ({
        ...t,
        ...LEGACY_TEMPLATE_DEFAULTS
    }));
    ...
    const githubTemplates = githubImportedTemplates_1.GITHUB_IMPORTED_TEMPLATES.map(t => ({
        ...t,
        ...LEGACY_TEMPLATE_DEFAULTS
    }));
    ...
    console.log('   📊 Breakdown: LocalStorage:', localStorageTemplates.length, '| Official:', officialTemplates.length, '| Static:', legacyStatic.length, '| GitHub File:', githubTemplates.length);
...
const syncLocalTemplatesToDB = async () => {
    console.log('Syncing local templates to DB... [DISABLED]');
    // DISABLED to prevent 400 errors on empty Supabase table
};
exports.syncLocalTemplatesToDB = syncLocalTemplatesToDB;
// ============================================
// HAZIR ŞABLONLAR
// ============================================
exports.AUTOMATION_TEMPLATES = [
    {
        ...
        blueprint: {
            ...
            nodes: [
                { id: 'wa-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Mesaj Alıcı', role: 'Webhook dinleyici', task: 'Gelen WhatsApp mesajlarını yakala', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'wa-2' }] },
                { id: 'wa-2', type: types_1.NodeType.AGENT_PLANNER, title: 'Niyet Analizi', role: 'AI Analiz', task: 'Mesajın amacını belirle (soru, şikayet, sipariş)', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'wa-3' }] },
                { id: 'wa-3', type: types_1.NodeType.LOGIC_GATE, title: 'Yönlendirici', role: 'Karar verici', task: 'SSS ise otomatik yanıt, karmaşık ise insan yönlendir', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'wa-4', condition: 'auto' }, { targetId: 'wa-5', condition: 'human' }] },
                { id: 'wa-4', type: types_1.NodeType.CONTENT_CREATOR, title: 'AI Yanıt Üretici', role: 'HuggingFace Mistral 7B', task: 'Bağlamsal ve nazik yanıt oluştur', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'wa-6' }] },
                { id: 'wa-5', type: types_1.NodeType.HUMAN_APPROVAL, title: 'İnsan Bildirimi', role: 'Slack/Email', task: 'Destek ekibine bildirim gönder', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'wa-6' }] },
                { id: 'wa-6', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Mesaj Gönderici', role: 'WhatsApp API', task: 'Yanıtı müşteriye gönder', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        },
        requiredApis: [
    ...
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'gr-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Yorum Alıcı', role: 'Google API', task: 'Yeni Google Business yorumlarını çek', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'gr-2' }] },
                { id: 'gr-2', type: types_1.NodeType.ANALYST_CRITIC, title: 'Sentiment Analizi', role: 'AI Analiz', task: 'Yorumun tonunu analiz et (olumlu/olumsuz/nötr)', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'gr-3' }] },
                { id: 'gr-3', type: types_1.NodeType.LOGIC_GATE, title: 'Yönlendirici', role: 'Karar', task: 'Olumsuz ise acil, olumlu ise standart yanıt', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'gr-4', condition: 'negative' }, { targetId: 'gr-5', condition: 'positive' }] },
                { id: 'gr-4', type: types_1.NodeType.HUMAN_APPROVAL, title: 'Acil Bildirim', role: 'Alert', task: 'Olumsuz yorum için yöneticiye bildirim', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'gr-6' }] },
                { id: 'gr-5', type: types_1.NodeType.CONTENT_CREATOR, title: 'AI Yanıt Üretici', role: 'HuggingFace', task: 'Kişiselleştirilmiş teşekkür/çözüm mesajı yaz', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'gr-6' }] },
                { id: 'gr-6', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Yanıt Gönder', role: 'Google API', task: 'Hazırlanan yanıtı yoruma gönder', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        },
        requiredApis: [
    ...
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'pt-1', type: types_1.NodeType.STATE_MANAGER, title: 'URL Listesi', role: 'Konfigürasyon', task: 'Takip edilecek ürün URL listesini al', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'pt-2' }] },
                { id: 'pt-2', type: types_1.NodeType.RESEARCH_WEB, title: 'Fiyat Scraper', role: 'Web Crawler', task: 'Her URL\'den güncel fiyatı çek', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'pt-3' }] },
                { id: 'pt-3', type: types_1.NodeType.ANALYST_CRITIC, title: 'Fiyat Karşılaştırıcı', role: 'Analiz', task: 'Önceki fiyatlarla karşılaştır, değişim hesapla', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'pt-4' }] },
                { id: 'pt-4', type: types_1.NodeType.LOGIC_GATE, title: 'Değişim Algılayıcı', role: 'Tetikleyici', task: 'Fiyat düştüyse veya çıktıysa bildir', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'pt-5' }] },
                { id: 'pt-5', type: types_1.NodeType.CONTENT_CREATOR, title: 'Strateji Önerici', role: 'AI', task: 'Fiyat stratejisi önerisi oluştur', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'pt-6' }] },
                { id: 'pt-6', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Bildirim Gönder', role: 'Email/Slack', task: 'Raporu ve önerileri gönder', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'sc-1', type: types_1.NodeType.RESEARCH_WEB, title: 'Trend Tarayıcı', role: 'Twitter/Instagram', task: 'Günün viral trendlerini bul', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'sc-2' }] },
                { id: 'sc-2', type: types_1.NodeType.ANALYST_CRITIC, title: 'Fırsat Analizi', role: 'AI', task: 'Markayla uyumlu trendleri filtrele', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'sc-3' }] },
                { id: 'sc-3', type: types_1.NodeType.CONTENT_CREATOR, title: 'Metin Yazarı', role: 'Copywriter AI', task: 'Platformlara özel caption yaz', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'sc-4' }] },
                { id: 'sc-4', type: types_1.NodeType.MEDIA_ENGINEER, title: 'Görsel Önerici', role: 'Design AI', task: 'Görsel konsepti ve prompt oluştur', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'sc-5' }] },
                { id: 'sc-5', type: types_1.NodeType.SOCIAL_MANAGER, title: 'İçerik Paketi', role: 'Export', task: 'Hazır paylaşım paketi oluştur', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'lh-1', type: types_1.NodeType.STATE_MANAGER, title: 'Hedef Tanımı', role: 'Konfigürasyon', task: 'Sektör, pozisyon, şirket büyüklüğü kriterleri', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'lh-2' }] },
                { id: 'lh-2', type: types_1.NodeType.RESEARCH_WEB, title: 'LinkedIn Tarayıcı', role: 'Scraper', task: 'Kriterlere uyan profilleri bul', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'lh-3' }] },
                { id: 'lh-3', type: types_1.NodeType.ANALYST_CRITIC, title: 'Lead Skorlama', role: 'AI Analiz', task: 'Potansiyeli değerlendir ve puanla', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'lh-4' }] },
                { id: 'lh-4', type: types_1.NodeType.CONTENT_CREATOR, title: 'Mesaj Hazırlayıcı', role: 'Copywriter', task: 'Kişiselleştirilmiş bağlantı mesajı yaz', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'lh-5' }] },
                { id: 'lh-5', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'CRM Export', role: 'Entegrasyon', task: 'Lead\'leri CRM\'e aktar', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'ar-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Veri Kaynakları', role: 'API Bağlantıları', task: 'GA, Facebook, Satış verilerini çek', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ar-2' }] },
                { id: 'ar-2', type: types_1.NodeType.STATE_MANAGER, title: 'Veri Birleştirici', role: 'ETL', task: 'Tüm verileri tek formatta birleştir', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ar-3' }] },
                { id: 'ar-3', type: types_1.NodeType.ANALYST_CRITIC, title: 'AI Yorumcu', role: 'HuggingFace Mistral 7B', task: 'Trendleri ve anomalileri analiz et', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ar-4' }] },
                { id: 'ar-4', type: types_1.NodeType.CONTENT_CREATOR, title: 'Rapor Yazıcı', role: 'Sunum', task: 'Yönetici özeti ve öneriler oluştur', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ar-5' }] },
                { id: 'ar-5', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Rapor Dağıtımı', role: 'Email/Slack', task: 'PDF raporu ilgili kişilere gönder', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'ca-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Borsa Bağlantıları', role: 'API', task: 'Binance, Coinbase, Kraken fiyatlarını real-time çek', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ca-2' }] },
                { id: 'ca-2', type: types_1.NodeType.TRADING_DESK, title: 'Fiyat Karşılaştırıcı', role: 'Hesaplama', task: 'Aynı coin için tüm borsaları karşılaştır', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ca-3' }] },
                { id: 'ca-3', type: types_1.NodeType.LOGIC_GATE, title: 'Karlılık Filtresi', role: 'Karar', task: 'İşlem ücretleri dahil kar %2+ mı?', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ca-4' }] },
                { id: 'ca-4', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Acil Bildirim', role: 'Push/SMS', task: 'Fırsat detaylarını anında gönder', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        },
        requiredApis: [
    ...
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'inv-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Sipariş Alıcısı', role: 'E-ticaret/CRM API', task: 'Yeni siparişleri yakala', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'inv-2' }] },
                { id: 'inv-2', type: types_1.NodeType.CONTENT_CREATOR, title: 'Fatura Üretici', role: 'PDF Generator', task: 'Sipariş verilerine göre fatura oluştur', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'inv-3' }] },
                { id: 'inv-3', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Fatura Gönderici', role: 'Email/SMS', task: 'Faturayı müşteriye gönder', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'inv-4' }] },
                { id: 'inv-4', type: types_1.NodeType.STATE_MANAGER, title: 'Ödeme Takibi', role: 'Database', task: 'Ödeme durumunu izle ve sakla', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'inv-5' }] },
                { id: 'inv-5', type: types_1.NodeType.LOGIC_GATE, title: 'Tardiye Detektörü', role: 'Karar', task: '15 gün gecikmiş mi?', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'inv-6' }] },
                { id: 'inv-6', type: types_1.NodeType.CONTENT_CREATOR, title: 'Hatırlatma Yazıcı', role: 'Personalization', task: 'Efektif ödeme talep mesajı yaz', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'inv-7' }] },
                { id: 'inv-7', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Hatırlatma Gönderici', role: 'Multi-channel', task: 'Email, SMS, WhatsApp ile gönder', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'amz-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Satış Verisi', role: 'Amazon SP API', task: 'Son 30 günün satış ve stok durumunu çek', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'amz-2' }] },
                { id: 'amz-2', type: types_1.NodeType.ANALYST_CRITIC, title: 'Satış Hızı Analizi', role: 'ML Model', task: 'Günlük satış hızını hesapla, trend bul', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'amz-3' }] },
                { id: 'amz-3', type: types_1.NodeType.LOGIC_GATE, title: 'Restock Karar', role: 'Algoritma', task: 'Tedarik süresi + arası X satış hızı = restock mu?', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'amz-4' }] },
                { id: 'amz-4', type: types_1.NodeType.CONTENT_CREATOR, title: 'Sipariş Oluşturucu', role: 'Veri Hazırlayıcı', task: 'Tedarikçiye gönderilecek sipariş hazırla', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'amz-5' }] },
                { id: 'amz-5', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Tedarikçi Bildir', role: 'Email/API', task: 'Sipariş detaylarını tedarikçiye gönder', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'amz-6' }] },
                { id: 'amz-6', type: types_1.NodeType.STATE_MANAGER, title: 'Sipariş Takibi', role: 'Database', task: 'Sipariş durumunu ve tarihini kaydet', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 're-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Site Ziyaretçi Takip', role: 'Webhook/Pixel', task: 'Ziyaretçi aktivitesini ve görüntülenen mülkleri kaydet', status: types_1.StepStatus.IDLE, connections: [{ targetId: 're-2' }] },
                { id: 're-2', type: types_1.NodeType.ANALYST_CRITIC, title: 'Lead Skorlama', role: 'AI', task: 'Ziyaretçi davranışından satın alma niyetini tahmin et', status: types_1.StepStatus.IDLE, connections: [{ targetId: 're-3' }] },
                { id: 're-3', type: types_1.NodeType.LOGIC_GATE, title: 'Yüksek Değerli mi?', role: 'Karar', task: 'Score 70+ puan ise qualified lead', status: types_1.StepStatus.IDLE, connections: [{ targetId: 're-4' }] },
                { id: 're-4', type: types_1.NodeType.CONTENT_CREATOR, title: 'Sunuş Taslağı', role: 'Copywriter', task: 'Kişiselleştirilmiş emlak sunumu taslağı oluştur', status: types_1.StepStatus.IDLE, connections: [{ targetId: 're-5' }] },
                { id: 're-5', type: types_1.NodeType.HUMAN_APPROVAL, title: 'Danışman Bildirimi', role: 'Slack/SMS', task: 'Danışmanı qualified lead ve sunuş taslağı ile bildir', status: types_1.StepStatus.IDLE, connections: [{ targetId: 're-6' }] },
                { id: 're-6', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Lead CRM Export', role: 'Integrasyon', task: 'Lead\'i CRM\'e aktar ve takip et', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    // ============== OVI VIDEO TEMPLATES ==============
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'ovi-1', type: types_1.NodeType.RESEARCH_WEB, title: 'Trend Tarayıcı', role: 'TikTok/Instagram', task: 'Günün viral trendlerini ve hook\'ları bul', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ovi-2' }] },
                { id: 'ovi-2', type: types_1.NodeType.CONTENT_CREATOR, title: 'Script Yazıcı', role: 'AI Copywriter', task: 'Viral hook ve script oluştur (5-10sn)', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ovi-3' }] },
                { id: 'ovi-3', type: types_1.NodeType.MEDIA_ENGINEER, title: 'OVI Video Üretici', role: 'OVI AI', task: 'Text-to-Video: Konuşan avatar + senkronize ses', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ovi-4' }] },
                { id: 'ovi-4', type: types_1.NodeType.ANALYST_CRITIC, title: 'Kalite Kontrol', role: 'AI Review', task: 'Video kalitesi, lip-sync ve ses kontrolü', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'ovi-5' }] },
                { id: 'ovi-5', type: types_1.NodeType.SOCIAL_MANAGER, title: 'Sosyal Medya Export', role: 'Multi-platform', task: 'TikTok, Reels, Shorts formatlarında dışa aktar', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'spk-1', type: types_1.NodeType.STATE_MANAGER, title: 'Mesaj Girdisi', role: 'Kullanıcı Input', task: 'Söylenmesini istediğin mesajı al', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'spk-2' }] },
                { id: 'spk-2', type: types_1.NodeType.CONTENT_CREATOR, title: 'Script Düzenleyici', role: 'AI Editor', task: 'Mesajı profesyonel formata dönüştür', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'spk-3' }] },
                { id: 'spk-3', type: types_1.NodeType.MEDIA_ENGINEER, title: 'OVI Avatar Üretici', role: 'OVI AI', task: 'Konuşan avatar videosu oluştur + lip-sync', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'spk-4' }] },
                { id: 'spk-4', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Video Export', role: 'MP4/WebM', task: 'Video dosyasını indir ve kaydet', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    {
        ...
        blueprint: {
            ...
            testConfig: { variables: [], simulateFailures: false },
            nodes: [
                { id: 'prd-1', type: types_1.NodeType.EXTERNAL_CONNECTOR, title: 'Ürün Verisi', role: 'E-ticaret API', task: 'Ürün bilgilerini ve görsellerini çek', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'prd-2' }] },
                { id: 'prd-2', type: types_1.NodeType.CONTENT_CREATOR, title: 'Tanıtım Scripti', role: 'Sales Copywriter', task: 'Ürün özelliklerinden satış scripti yaz', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'prd-3' }] },
                { id: 'prd-3', type: types_1.NodeType.MEDIA_ENGINEER, title: 'OVI Image-to-Video', role: 'OVI AI I2V', task: 'Ürün görselini videoya dönüştür + seslendirme', status: types_1.StepStatus.IDLE, connections: [{ targetId: 'prd-4' }] },
                { id: 'prd-4', type: types_1.NodeType.SOCIAL_MANAGER, title: 'Çoklu Format Export', role: 'Video Processor', task: 'Instagram, TikTok, YouTube Shorts formatları', status: types_1.StepStatus.IDLE, connections: [] }
            ]
        }
    },
    // ============================================
...
const getTemplateById = async (id) => {
    const all = await (0, exports.getTemplates)();
    return all.find(t => t.id === id);
};
exports.getTemplateById = getTemplateById;
const getTemplatesByCategory = (category) => {
    return exports.AUTOMATION_TEMPLATES.filter(t => t.category === category);
};
exports.getTemplatesByCategory = getTemplatesByCategory;
const searchTemplates = (query) => {
    const lowerQuery = query.toLowerCase();
    return exports.AUTOMATION_TEMPLATES.filter(t => t.name.toLowerCase().includes(lowerQuery) ||
        t.description.toLowerCase().includes(lowerQuery) ||
        t.tags.some(tag => tag.toLowerCase().includes(lowerQuery)));
};
exports.searchTemplates = searchTemplates;
// Helper to generate dynamic nodes based on category
const generateDefaultNodes = (template) => {
    const nodes = [];
    ...
    if (category === 'content' || category === 'video') {
        nodes.push({
            id: 'step-2',
            type: types_1.NodeType.RESEARCH_WEB,
        ...
        nodes.push({
            id: 'step-3',
            type: types_1.NodeType.CONTENT_CREATOR,
        ...
        nodes.push({
            id: 'step-4',
            type: types_1.NodeType.SOCIAL_MANAGER,
            title: 'Yayınlama',
            role: 'Social Media',
        ...
        nodes.push({
            id: 'step-3',
            type: types_1.NodeType.ANALYST_CRITIC,
            title: 'Veri Analizi',
            role: 'Analyst',
        ...
        nodes.push({
            id: 'step-3',
            type: types_1.NodeType.AGENT_PLANNER,
            title: 'Niyet Analizi',
            role: 'AI Brain',
        ...
        nodes.push({
            id: 'step-3',
            type: types_1.NodeType.LOGIC_GATE,
...
const createBlueprintFromTemplate = (template) => {
    const blueprint = {
...
exports.createBlueprintFromTemplate = createBlueprintFromTemplate;
