...
exports.nicheFinderService = {
    async scanForOpportunities() {
        ...
        const apiKey = import.meta.env.VITE_GROQ_API_KEY; // Legacy check, service handles it too.
        // Mevcut şablonları listele
        const existingTemplates = templateService_1.AUTOMATION_TEMPLATES.map(t => t.name).join(', ');
        ...
        try {
            ...
            const ideas = await groqService_1.groqService.generateJSON('You are a JSON generator. Always output valid JSON only.', prompt);
            if (!ideas || !Array.isArray(ideas)) {
                throw new Error('Groq returned invalid data');
            }
            const pIdeas = ideas.map((idea, index) => ({
