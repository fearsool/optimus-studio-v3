...
import { Policy, ActionType } from '../core/optimusTypes';

// ============================================
// FACTORY POLICIES
// ============================================

export const FACTORY_POLICIES: Policy[] = [
    // ==========================================
    ...
    // ==========================================
    {
        ...
        condition: (ctx) => {
            ...
            const aiApproved = ctx.decision.params?.nemotronResult?.verdict === 'PRODUCE';
    ...
    // ==========================================
    {
        ...
        condition: (ctx) => {
            ...
            const isCritical = ['STOP', 'DELETE', 'CRISIS'].some(
                a => ctx.intent.action.includes(a)
            );
    ...
    // ==========================================
    {
        ...
        condition: (ctx) => {
            const isNew = ['CREATE', 'START', 'PRODUCE'].some(
                a => ctx.intent.action.includes(a)
            );
    ...
    // ==========================================
    {
        ...
        condition: (ctx) => {
            ...
            const dailyCount = ctx.decision.params?.dailyProductCount || 0;
    ...
    },
    {
        ...
        condition: (ctx) => {
            const lastProduction = ctx.decision.params?.lastProductionTime;
            if (!lastProduction) return false;
...
export function getPolicyById(id: string): Policy | undefined {
    return FACTORY_POLICIES.find(p => p.id === id);
}

/**
 * Get policies by action type
 */
export function getPoliciesByAction(action: ActionType): Policy[] {
    return FACTORY_POLICIES.filter(p => p.action === action);
}
...
export function getEnabledCount(): number {
    return FACTORY_POLICIES.filter(p => p.enabled).length;
}
