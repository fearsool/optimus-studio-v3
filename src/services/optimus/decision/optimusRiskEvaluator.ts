...
interface RiskFactor {
    name: string;
    weight: number;
    evaluate: (intent: ResolvedIntent) => number; // 0-100
}
...
const RISK_FACTORS: RiskFactor[] = [
    // Low confidence = higher risk
    {
        ...
        weight: 0.25,
        evaluate: (intent) => {
            if (intent.confidence >= 0.8) return 0;
            if (intent.confidence >= 0.6) return 30;
            if (intent.confidence >= 0.4) return 60;
            return 100;
        }
    },
    ...
    // Financial impact
    {
        ...
        evaluate: (intent) => {
            ...
            if (financialIntents.includes(intent.action)) return 70;
            return 0;
        }
    },
    ...
    // Destructive actions
    {
        ...
        evaluate: (intent) => {
            ...
            if (destructiveKeywords.some(k => intent.action.includes(k))) return 80;
            return 0;
        }
    },
    ...
    // Unknown intent
    {
        ...
        weight: 0.15,
        evaluate: (intent) => {
            if (intent.action === 'UNKNOWN') return 90;
            return 0;
        }
    },
    ...
    // System stress
    {
        ...
        evaluate: () => {
            ...
            if (status.health === 'CRITICAL') return 100;
            if (status.health === 'DEGRADED') return 50;
            return 0;
        }
    },
...
interface Mitigation {
    condition: (intent: ResolvedIntent, score: number) => boolean;
    action: string;
    reduction: number;
}
...
const MITIGATIONS: Mitigation[] = [
    ...
    {
        condition: () => optimusCore.getMode() === 'CRISIS',
...
class OptimusRiskEvaluator {
     ...
     */
    async evaluate(intent: ResolvedIntent): Promise<RiskAssessment> {
     ...
     */
    isHighRisk(intent: ResolvedIntent): boolean {
...
export const riskEvaluator = new OptimusRiskEvaluator();
