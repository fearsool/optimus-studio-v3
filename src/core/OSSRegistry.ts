...
export interface OSSComponent {
    ...
    integrationStatus: 'integrated' | 'planned' | 'optional';
    replaces?: string;  // What cloud/API service it replaces
    notes?: string;
}
...
// =============== OSS REGISTRY ===============
export const OSS_REGISTRY: OSSComponent[] = [
    // ═══════════════ LLM RUNTIME ═══════════════
    {
        name: 'Ollama',
...
export function getByCategory(category: OSSCategory): OSSComponent[] {
    return OSS_REGISTRY.filter(c => c.category === category);
}

export function getIntegrated(): OSSComponent[] {
    return OSS_REGISTRY.filter(c => c.integrationStatus === 'integrated');
}

export function getPlanned(): OSSComponent[] {
    return OSS_REGISTRY.filter(c => c.integrationStatus === 'planned');
}

export function findReplacement(cloudService: string): OSSComponent | undefined {
    return OSS_REGISTRY.find(c =>
        c.replaces?.toLowerCase().includes(cloudService.toLowerCase())
    );
}
